...
export const CartSlider = ({
  ...
  return (
    <div
      className={`flex justify-end items-start min-h-screen w-full fixed left-0 right-0 top-0 transition-opacity duration-75 overflow-hidden bg-black/30 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? "opacity-100 pointer-events-auto z-100" : "opacity-0 pointer-events-none z-0 delay-200"}`}
    >
      <div
        className={`absolute right-0 top-0 h-full w-full md:w-100 lg:w-105 bg-white p-4 flex flex-col justify-start items-start transition-transform duration-200 delay-100 ease-[cubic-bezier(0.22,1,0.36,1)] ${isOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <div className="">
        ...
        <div className="[&_.button]:bg-black [&_.button]:text-white! [&_.button]:font-bold [&_.button]:block [&_.button]:w-full [&_.button]:rounded-full [&_.button]:mt-2 [&_.button]:p-3 [&_.button]:text-center [&_.button]:text-lg py-4 border-t border-neutral-300 w-full mt-auto">
            ...
            <h3 className="flex justify-between items-center font-bold text-xl">
              ...
              <span>
                {formatPrice(
                  ...
                  cart.cart?.cost.totalAmount.currencyCode ?? "USD",
                )}
              </span>
            </h3>
          ...
          <a className="button" href={cart.cart?.checkoutUrl}>
            Checkout Now
          </a>
        </div>
      </div>
